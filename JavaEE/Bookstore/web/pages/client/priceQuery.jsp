    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(".addToCart").click(function () {
                //直接请求，即改变地址栏地址
                location.href="<%= basePath%>" + "cartServlet?action=goodsAdd&&goodsId=" + $(this).attr("goodsId") + "&&currentPage=" + ${requestScope.currentPage};
            });
            //给价格筛选绑定单击事件
...
<div id="page_nav">
    ...
    <form action="clientBookServlet" method="post">
        <input type="hidden" name="action" value="queryPagesByPrice"/>
        <a href="clientBookServlet?action=queryPagesByPrice&max=${requestScope.max}&min=${requestScope.min}">首页</a>

        <% if ((Integer)(request.getAttribute("currentPage")) - 1 > 0) { %>
        <a href="clientBookServlet?action=queryPagesByPrice&currentPage=${requestScope.currentPage - 1}&max=${requestScope.max}&min=${requestScope.min}">上一页</a>
        <a href="clientBookServlet?action=queryPagesByPrice&currentPage=${requestScope.currentPage - 1}&max=${requestScope.max}&min=${requestScope.min}">${requestScope.currentPage - 1}</a>
        <% }%>
        ...
        <a href="clientBookServlet?action=queryPagesByPrice&currentPage=${requestScope.currentPage + 1}&max=${requestScope.max}&min=${requestScope.min}">${requestScope.currentPage + 1}</a>
        <a href="clientBookServlet?action=queryPagesByPrice&currentPage=${requestScope.currentPage + 1}&max=${requestScope.max}&min=${requestScope.min}">下一页</a>
        <% }%>

        <a href="clientBookServlet?action=queryPagesByPrice&currentPage=${requestScope.pageNum}&max=${requestScope.max}&min=${requestScope.min}">末页</a>
        <!-- 记录价格参数-->
        <input type="hidden" name="max" value="${requestScope.max}"/>
        <input type="hidden" name="min" value="${requestScope.min}"/>
        <!-- 另一种方法，不使用form，使用JS中的 location对象，可以设置和获取地址栏地址-->
